#this file is the code for early fusion neural network.
#By default, we use tunable memristor neuron, but you can check the difference of accuracy by setting mx=1 to simulate the untunable memristor neuron.
import os
...
data = np.array([])
x=2.5                  # gate voltage：it can be set between -3V and 3V.the basic gate voltage=-3V
mx=(-x*x+12*x+65)/20 # spiking current ratio(The ratio of excitement to inhibition), ATTENTION:you can set mx=1 to simulate the untunable memristor neuron.
batch_size = 1000    # a batch contains how many samples:2000total=1000audio+1000mnist
...
def audiodatabuild():
    ...
    X = torch.load('E:/tensor/myTensorX.pt') # The file has been provided, but the relative path needs to be modified
    global Y1,Y2,Y   # labels of training set, testing set, data set
    Y = torch.load('E:/tensor/myTensorY.pt') # The file has been provided, but the relative path needs to be modified
...
DATA_PATH = 'E:\MNIST' # The file has not been provided, you can easily download it from the internet  
...
class SpikingNet(nn.Module):
    ...
    def forward_through_time(self, x):
        ...
        for _ in range(self.n_time_steps):
            ...
            #layer1_output=torch.mul(layer1_output,layer1_state)
